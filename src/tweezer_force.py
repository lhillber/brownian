...
class Domain:
    ...
    ----------
        axes : List of 1D arrrays
            Three 1D coordinate axes defined by each row of meshspec.
            These are the three edges of the domain volume.

        deltas : list
            Three step sizes of each coordinate axis
...
class Tweezers:
    """
    Counter propagating dual beam optical tweezer force calculations and
    visualizations.

    Mie scattering calculations based on the T matrix method
    and vector spherical harmonic basis functions are provided by the Optical
    Tweezers Toolbox (OTT) add on for Matlab. This class provides a Python
    interface to a small and specific set of OTT functions for calculating
    the force on a spherical dielectric particle trapped by two counter
    propagating, cross polarized, tightly focused TEM00-mode laser beams.
    ...
    def file_path(self, der=None, nickname=None):
        ...
        if nickname is None:
            nickname = hash_state(self.params, uid_keys=self.params.keys())
        ...
        """
            ...
            ----------
                ...
                *max : float
                ...
                N* : int
                    The number of points along the x, y, and x axes
    ...
    def make_beam(self, name, NA, polarization, angle, add_name=True):
        """
        ...
        ----------
            ...
            NA : float
                Numerical aperture of the focusing element. Default is 0.7.
    ...
    def find_min(self, name="net", domain=None):
        ...
        if domain is None:
            domain = self.domain
        F = self.interpolate_force(points=domain.points, name=name)
        mindex = np.argmin(np.sum(F*F, axis=1))
        return domain.points[mindex]


    def fit_min(self, name="net", domain=None):
        """
        Fit the global minimum in force magnitude within a given domain
        to a 3D linear function.
        ...
        if domain is None:
            domain = self.domain
        qs = domain.axes
        Fs = [self.interpolate_force(points=[q], name=name) for q in qs]
        q0s = self.find_min(name, domain)
        def func(q, q0, k):
            return -k * (q - q0)
        location = np.zeros((2, 3))
        strength = np.zeros((2, 3))
        for i, (q, q0, F) in enumerate(zip(qs, q0s, Fs)):
            popt, pcov = curve_fit(func, q, F.ravel(), p0=(q0, 10*fN/nm))
    ...
    def load(self, fpath=None):
        ...
        beam_args = ["NA", "polarization", "angle"]
    ...
    def plot_slices(
        ...
        ----------
            ...
            domain : Domain object
                ...
                used to define the interpolation. Esenti
        ...
        **plot_kwargs,
        ...
        for k, v in zip("xyz", line):
            ...
            if v == "*":
                ...
                points[::, idx] = v
                label += " ${} = {}$".format(k, v/unit)
        ylabel = "$F_{%s}$"
        ax.plot(vars/unit, interp(points)/Funit, label=label, **plot_kwargs)
        if legend:
            ax.legend(fontsize=10)
        ax.set_ylabel(ylabel % "xyz"[comp])
        ax.set_xlabel(r"$%s$" % "xyz"[vari])
        ax.label_outer()
        return fig, ax
