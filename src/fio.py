...
class IO:
    ...
    def __init__(self, recalc=False, nickname=None, der=None, uid_keys=[]):
        self._nickname = nickname
        ext = type(self).__name__.lower()
        if der == None:
            der = path.join(base_der, "data", ext)
    ...
    def nickname(self):
        if self._nickname == None:
            return self.uid()
        fname = path.join(der, self.nickname) + "." + ext

    @property
    def fname(self):
        return path.join(self.der, self.nickname) + "." + self.ext

    @property
    def rel_fname(self):
        return path.relpath(self.fname, base_der)



    def uid(self):
        return hash_state(self.__dict__, self.uid_keys)
...
if __name__ == "__main__":
    ...
    class Level1(IO):
        def __init__(self, params, der=None, nickname=None, recalc=False, save=True):
            ...
            # adds attributes recalc, loaded, and der
            super().__init__(der=der, nickname=nickname, recalc=recalc)
            if recalc:
