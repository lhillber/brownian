...
from brownian import (partition, PSD, MSD, ACF, abc_guess, psdfunc, msdfunc, pacfunc,
                      vacfunc, dataplot)
...
class TimeSeries:
    def __init__(self, fname=None, rate=None, verbose=False):
    ...
    def detrend(self, key, mode='constant', breakpoints=0, inplace=False):
        t, x = self.get_tx(key)
        x2 = detrend(x, type=mode, bp=breakpoints)
        if inplace:
            self.x = x2
        return x2

    def PSD(self, key="x", taumax=None, detrend="linear", window="hamming"):
        # split data into partitions of length tmax
        if taumax is None:
            taumax = t[-1]
        t, x = get_tx(key)
        r = self.r(key)
        Npts = min(t.size, int(taumax * r))
        if self.verbose:
            print("Calculating PSD...")
        tpart, xpart = self.partition(key, taumax=taumax)
        Navg = len(xpart)
        Npts = len(xpart[0])
        if self.verbose:
